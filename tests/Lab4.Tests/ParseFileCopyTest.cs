...
public class ParseFileCopyTest
{
    [Theory]
    ...
    {
        var parser = new StringParser($"connect {connectPath} -m test file copy {sourcePath} {destinationPath}");
        ...
        Assert.IsType<CommandResult.FileCopy>(testFileSystem.Result);
        if (testFileSystem.Result is not CommandResult.FileCopy result) return;
