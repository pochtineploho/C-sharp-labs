...
{
    [Theory]
    [MemberData(nameof(Lab5TestDataGenerator.DepositTestData), MemberType = typeof(Lab5TestDataGenerator))]
    public async Task TestDeposit(long balance, long deposit)
    {
        ...
        var service = new BalanceService(
            new BankAccountAdapter(bankRepository),
            new OperationHistoryAdapter(historyRepository));
        ...
        BinaryResult result = await service.Deposit("6969152203847379", deposit);

        Assert.Equal((balance + deposit).ToString(), result.Message);
    }
}